...
`nest build` is a wrapper on top of the standard `tsc` compiler or `swc` compiler (for [standard projects](./overview#project-structure)) or the webpack bundler using the `ts-loader` (for [monorepos](./overview#project-structure)). It does not add any other compilation features or steps except for handling `tsconfig-paths` out of the box. The reason it exists is that most developers, especially when starting out with Nest, do not need to adjust compiler options (e.g., `tsconfig.json` file) which can sometimes be tricky.

See the [nest build](./usages#nest-build) documentation for more details.
...
See the [nest start](./usages#nest-start) documentation for more details.
